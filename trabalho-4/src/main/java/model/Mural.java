...
public class Mural implements Serializable {
    ...
    public Mural() {
        this(10, 10, new Artista("Artista"));
    }
    ...
    public synchronized void criar(int largura, int altura, String dono) {
        this.largura = largura;
        this.altura  = altura;
        this.dono    = new Artista(dono == null || dono.isBlank() ? "Artista" : dono);
        this.pixels.clear();
    }
    ...
    public synchronized String visualizarMuralAnsi() {
        ...
        for (int y = 0; y < altura; y++) {
            for (int x = 0; x < largura; x++) {
                Pixel p = buscarPixel(x, y);
                sb.append(p == null ? " ." : simboloCor(p.getCor()));
            }
            sb.append("\n");
        }
        return sb.toString();
    ...
    public synchronized String visualizarMuralTexto() {
        ...
        for (int y = 0; y < altura; y++) {
            for (int x = 0; x < largura; x++) {
                Pixel p = buscarPixel(x, y);
                sb.append(p == null ? " ." : " " + p.getCor().charAt(0));
            }
            sb.append("\n");
        }
        return sb.toString();
    ...
    private String simboloCor(String cor) {
        return switch (cor.toUpperCase()) {
            case "VERMELHO" -> "\u001B[41m  \u001B[0m";
            case "AZUL"     -> "\u001B[44m  \u001B[0m";
            case "VERDE"    -> "\u001B[42m  \u001B[0m";
            case "AMARELO"  -> "\u001B[43m  \u001B[0m";
            case "PRETO"    -> "\u001B[40m  \u001B[0m";
            case "BRANCO"   -> "\u001B[47m  \u001B[0m";
            case "ROSA"     -> "\u001B[45m  \u001B[0m";
            case "MARROM"   -> "\u001B[48;5;94m  \u001B[0m";
            case "LARANJA"  -> "\u001B[48;5;208m  \u001B[0m";
            default         -> " ?";
        };
    }
