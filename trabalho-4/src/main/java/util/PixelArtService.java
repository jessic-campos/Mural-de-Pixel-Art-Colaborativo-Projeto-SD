...
import model.Pixel;
import model.Pincel;
import queue.EventoFila;
import queue.FilaPersistente;
...
public class PixelArtService {
    ...
    // ---- Produtores (publicam eventos na fila) ----
    ...
    public synchronized String visualizarMuralJson(boolean ansi) {
        ...
        return "{\"ok\":true,\"mural\":\"" + JsonUtil.escape(visual) + "\",\"mensagensPendentes\":"
            + fila.tamanho() + ",\"dados\":" + mural.toJson() + "}";
    }
    ...
    // ---- Consumidor (usado pelo WorkerFila) ----
    ...
    private void aplicarEventoNoMural(Mural mural, EventoFila evento) {
        switch (evento.getTipo()) {
            case "CRIAR_MURAL"     -> mural.criar(evento.getLargura(), evento.getAltura(), evento.getDono());
            case "PINTAR_PIXEL"    -> mural.pintarPixel(evento.getX(), evento.getY(), evento.getCor());
            case "APAGAR_PIXEL"    -> mural.apagarPixel(evento.getX(), evento.getY());
            case "APLICAR_PINCEL"  -> pincel.aplicarArea(mural, evento.getX1(), evento.getY1(),
                                                         evento.getX2(), evento.getY2(), evento.getCor());
            case "APLICAR_BORRACHA"-> borracha.aplicar(mural, evento.getX(), evento.getY(), "BRANCO");
            default                -> System.out.println("[WORKER] Evento desconhecido ignorado: " + evento.getTipo());
        }
    }
}
